...
var KTChartsWidget8 = function () {
    ...
    return {
        init: function () {    
            ...
            initChart(dailyChart, '#rk_chart_widget_daily_toogle', '#dailyChart', data1, false);
            initChart(weeklyChart, '#rk_chart_widget_weekly_toogle', '#kt_chart_widget_8_month_chart', data2, true);    

            // Update chart on theme mode change
            var handlerId = KTThemeMode.on("kt.thememode.change", function() {
                if (dailyChart.rendered) {
                    dailyChart.self.destroy();
                }

                if (weeklyChart.rendered) {
                    weeklyChart.self.destroy();
                }

                initChart(dailyChart, '#rk_chart_widget_daily_toogle', '#kt_chart_widget_8_week_chart', data1, dailyChart.rendered);
                initChart(weeklyChart, '#rk_chart_widget_weekly_toogle', '#kt_chart_widget_8_month_chart', data2, weeklyChart.rendered);  
            });
        }   
    }
}();
