"use strict";
const KTAppExpeditionsInProgressListing = (function () {
    let table,
        d_table,
        flatpickr_object,
        date_start,
    ...
    return {
        init: function () {
            ...
            (   (d_table = $(table).DataTable({
                ...
                document.querySelector('[data-kt-expeditions-in-progress-filter="search"]')
                .addEventListener("keyup", function (e) {
                    d_table.search(e.target.value).draw();
                }),
                (() => {
                    const order_filter_status = document.querySelector('[data-kt-expeditions-in-progress-filter="status"]');
                    $(order_filter_status).on("change", (e) => {
                        let value = e.target.value;
                        "all" === value && (value = ""),
                        d_table.column(5).search(value).draw();
                    });
                })(),
                ...
                document.querySelector('[href="#rk_tab_expeditions_in_progress"]')
...
KTUtil.onDOMContentLoaded(function () {
    KTAppExpeditionsInProgressListing.init();
});
